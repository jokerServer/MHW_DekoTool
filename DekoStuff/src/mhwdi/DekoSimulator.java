...
public class DekoSimulator {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Long> dekoCountMap = simInfo.stream().collect(Collectors.groupingBy(s -> s.getLastMissingDeko().getDekoName(), Collectors.counting()));
        //Entry<String, Long> maxLast = dekoCountMap.entrySet().stream().max((p1, p2) -> Long.compare( p1.getValue(), p2.getValue())).orElseThrow(RuntimeException::new);
        ...
        System.out.println("The last deko type required an average of " + lastAvgSteps / (RUNS * 1f) + " steps to collect the last " + lastAvgSize / (RUNS * 1f) + " missing dekos");
        dekoCountSort.stream().forEach(s -> System.out.println(s.getKey() + " in " + s.getValue() + " runs (" + (s.getValue() * 100f) / (RUNS * 1f) + "%)"));
    }
}
