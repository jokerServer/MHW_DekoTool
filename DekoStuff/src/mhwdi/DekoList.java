...
//    }
    ...
    public final void reloadCSVv2() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(csvDekoFile))) {
            ...
            while ((csvLine = br.readLine()) != null && !"".equals(csvLine)) {
                ...
                if (german) {
                    csvLineDeko = new Dekoration(
                            /* rownum */csvLineCount,
                            /* owned  */ Integer.parseInt(csvColumns[0]),
                            /* tbs    */ 0,
                            ...
                            /*dekolvl */ Integer.parseInt(csvColumns[7]),
                            /*dekorar */ Integer.parseInt(csvColumns[8]),
                            /*weight  */ Integer.parseInt(csvColumns[9]),
                            /* myst%  */ Float.parseFloat(csvColumns[10]),
                            /* glow%  */ Float.parseFloat(csvColumns[11]),
    ...
    public Dekoration[] findMatching(String searchText, Boolean useDekoName, Boolean useSkillName, Boolean useSkillDesc) {
        return dekoList.stream().filter(s
                -> (useDekoName && s.getDekoName().toLowerCase().contains(searchText))
    ...
    public Dekoration[] findExcess() {
        return dekoList.stream().filter(s -> s.getOwned() > s.getSkillMaxLvl())
                .sorted((x, y) -> Integer.compare((x.getSkillMaxLvl() - x.getOwned()) * DEKO_RARITY_SMELTPOINTS.get(x.getDekoRarity()),
    ...
    public Dekoration[] findSmeltingSetup(int wantedPoints) {
        ...
        if (neededPoints > 0) {
            System.out.println("You dont have enough decorations to smelt efficiently, missing points: " + neededPoints + "/" + wantedPoints);
        }
    ...
    public Dekoration getDekoByMatch(String searchText, Boolean useDekoName, Boolean useSkillName, Boolean useSkillDesc) {
